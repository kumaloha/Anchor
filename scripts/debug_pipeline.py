...
Pipeline 完整调试脚本
=====================
逐步执行通用判断/内容提取/事实验证，将每步的详细输出写入 debug_output.txt。
...
async def run():
    ...
    from anchor.chains.general_assessment import assess_post as classify_post
    ...
    for pid in POST_IDS:
        async with AsyncSessionLocal() as session:
            ...
            section(f"通用判断 — post_id={pid}  [{yr}年]  {rp.author_name}")
            ...
            section(f"内容提取 — post_id={pid}  [{yr}年]")
    ...
    async with AsyncSessionLocal() as session:
        r = await run_verification(CURRENT_POST_ID, session)
