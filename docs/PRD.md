...
Anchor 负责**数字化**——从非结构化文本中提取**ExtractionNode**（领域特定的结构化实体）和**ExtractionEdge**（节点间的关系），并通过事实核查验证可信度。每篇文章独立提取，不做跨文章归一化（归 Axion）。

Anchor 是三层投资分析系统的第一层：

```
文章/文件 → [Anchor 数字化] → ExtractionNode + ExtractionEdge（per-article 结构化提取）
                                      ↓
...
**Anchor 的职责边界**：只做 per-article 的信息提取和结构化（数字化），不做跨文章归一化、世界建模、价格预测或投资决策。产出的 ExtractionNode/Edge 供下游 Axion 消费，由 Axion 完成模型化（知识图谱归一化、传导链构建等）。
...
- **v8.2：明确职责边界 — Anchor 只做 Layer 1 提取，Layer 2（跨文章归一化）归 Axion**

**v8 核心改进（相比 v7）：**
- **统一数据模型**：所有领域共用 `ExtractionNode` + `ExtractionEdge` 两张表，替代 7 实体表 + 5 专用表
...
└──────────────────────────────┬───────────────────────────────────┘
           ...
           └────────────────────┬─────────────────────┘
              ...
              │  Call 2: 发现边 + 生成摘要                 │
              │                                         │
              │  → ExtractionNode 表 + ExtractionEdge 表                     │
              │                                         │
              │  支持 6 个领域：                           │
...
┌─────────────────────────────────────────────────────────┐
│  Anchor（数字化）                                          │
│  文章/文件 → ExtractionNode + ExtractionEdge（per-article） │
│                                                           │
│  产出：                                                    │
│  ├── policy 节点+边（政策文件）                              │
│  ├── industry 节点+边（产业研究）                            │
...
├─────────────────────────────────────────────────────────┤
│  Axion（模型化）— 独立项目                                   │
│  Anchor ExtractionNode/Edge + 市场数据 → 知识图谱归一化 + 传导链 → 价格预测 │
├─────────────────────────────────────────────────────────┤
│  Polaris（量化投资）— 独立项目                               │
│  Axion 预测 + 当前定价 → 投资决策                            │
└─────────────────────────────────────────────────────────┘
...
**Anchor 的输出接口**：所有节点和边通过 SQLite DB 暴露（`extraction_nodes` + `extraction_edges` 表），Axion 直接读取。

**调用方向**：Axion 调用 Anchor（读取 DB / import 模块），Anchor 不依赖 Axion。
...
extraction_nodes (
    ...
    claim           TEXT,                         -- 主要内容（≤300字）
    summary         VARCHAR,                     -- 短摘要（≤30字）
    abstract        VARCHAR,                     -- 一句话总结（≤100字）
    metadata_json   TEXT,                        -- 领域特定扩展数据（JSON）
    valid_from      DATE,                        -- 生效日期（LLM 判断）
    valid_until     DATE,                        -- 失效日期（LLM 判断）
    authority       INTEGER,                     -- 权威等级：0=一手信息，其他=作者 credibility_tier
...
### 7.3 ExtractionEdge 表（提取边）

```sql
extraction_edges (
    id                INTEGER PRIMARY KEY,
    source_node_id    INTEGER FK → extraction_nodes.id,  -- 源节点
    target_node_id    INTEGER FK → extraction_nodes.id,  -- 目标节点
    edge_type         VARCHAR DEFAULT 'causes',  -- 12种有类型边（见下方）
    note              VARCHAR,                   -- ≤80字说明
    added_by_post_id  INTEGER FK → raw_posts.id, -- 来源文章
    authority         INTEGER,                   -- 权威等级
    created_at        DATETIME
)

-- 12 种边类型：causes | produces | derives | supports | contradicts |
...
extract_generic_write(raw_post, session, domain, compute_result)
  │
  ├── 清除旧数据：DELETE ExtractionNode/Edge WHERE raw_post_id = ?
  ├── 写入 ExtractionNode 表
  ├── 写入 ExtractionEdge 表
...
├── models.py                       # ExtractionNode + ExtractionEdge + DOMAIN_NODE_TYPES + 基础设施表
...
- **职责边界**：Anchor 只做 per-article 的数字化（提取 + 结构化）。跨文章归一化、产业建模（周期定位、传导链）属于 Axion；定价判断和投资决策属于 Polaris
- **数字化 ≠ 模型化**：Anchor 提取 ExtractionNode/Edge（per-article 文本结构化），Axion 做跨文章知识图谱和深度分析
...
- [x] **权威等级字段**：ExtractionNode/Edge.authority（0=一手信息，1+=作者 tier）
...
- [x] **职责边界明确化**：Anchor 只负责 Layer 1（per-article 提取），Layer 2（跨文章知识图谱、节点归一化）归 Axion
- [x] **移除 Knowledge 层代码**：删除 KnowledgeNode/Edge/Source 模型和 canonicalize 模块
- [x] **Ollama 本地模型支持**：通过原生 `/api/chat` 端点支持本地 LLM
...
> 文档状态：v8.2（2026-03-15），Anchor 只做 Layer 1 提取，Layer 2 归 Axion
