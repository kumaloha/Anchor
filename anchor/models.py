"""
Anchor 核心数据模型（v8.4 — 两层 Node/Edge + 政策模型 + 公司模型）
==================================================================
...
公司模型（巴菲特标准）：
  CompanyProfile / CompanyNarrative
  FinancialStatement / FinancialLineItem
    statement_type: income|balance_sheet|cashflow|equity|tax_detail|sbc_detail
  DownstreamSegment — 下游（每客户每期一行，segment 可选，含 backlog）
  UpstreamSegment — 上游（每供应商每期一行，segment 可选，含采购义务）
  GeographicRevenue — 地域收入
  NonFinancialKPI — 非财务KPI
  DebtObligation — 债务/租赁义务明细
  Litigation — 诉讼/或有事项
  ExecutiveCompensation — 管理层薪酬
  RelatedPartyTransaction — 关联交易
  TechInsight — 技术洞察（论文/专利共用：问题→方案→效果→局限 + 演进链 DAG）
  PatentRight — 专利法律权利（专利独有：claims, assignee, dates, status）
  PatentCommercial — 专利商业化（许可/诉讼/转让）
