...
========================
三条链路：
  Chain 1 — 逻辑提炼（URL → 六实体提取）
  Chain 2 — 作者分析（档案 + 立场）
  Chain 3 — 验证（Fact/Assumption/ImplicitCondition/Conclusion/Prediction）

用法：
  python run_v4_test.py "https://x.com/RayDalio/status/XXX"
  python run_v4_test.py "https://x.com/RayDalio/status/XXX" --chain 1  # 只跑 Chain 1
"""
...
async def run_test(url: str, only_chain: int | None = None):
    ...
    from anchor.chains import run_chain1, run_chain2, run_chain3
    ...
    if only_chain is None or only_chain == 1:
        _sep("Chain 1 — 逻辑提炼（六实体提取）")
        async with AsyncSessionLocal() as session:
            c1 = await run_chain1(url, session)
        ...
        async with AsyncSessionLocal() as session:
            ...
            assert rp, f"RawPost not found for URL={url}. Run Chain 1 first."
            raw_post_id = rp.id
    ...
    if (only_chain is None or only_chain == 2) and author_id:
        _sep("Chain 2 — 作者分析")
        async with AsyncSessionLocal() as session:
            c2 = await run_chain2(author_id, session)
    ...
    if only_chain is None or only_chain == 3:
        _sep("Chain 3 — 验证")
        async with AsyncSessionLocal() as session:
            c3 = await run_chain3(raw_post_id, session)
