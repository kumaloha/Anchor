...
|---------------------|---------|-------------|
| (expert, 事实) | Tavily + LLM 搜索核实 | credible \| vague \| unreliable \| unavailable |
...
```python
VERIFIABLE_TYPES = {
    ("expert", "事实"):   verify_fact,       # Tavily + LLM 搜索核实
    ("expert", "判断"):   derive_verdict,    # 从支撑边推导
    ("expert", "预测"):   monitor_prediction,# 时间窗口 + Tavily + LLM
...
│
├── chains/                          # 三条链路编排（入口）
│   ├── content_extraction.py        # 内容提取 — Node/Edge 读写
│   ├── general_assessment.py        # 通用判断 — 2D分类 + resolve_content_mode()
│   ├── fact_verification.py         # 事实验证 — 注册表式验证
│   └── prompts/
...
│   ├── rss.py                       # RSS/Atom
│   ├── web.py                       # 通用 Web（BeautifulSoup + arXiv→ar5iv）
│   └── manager.py                   # 采集轮询调度
│
├── monitor/                         # 订阅监控
...
│
├── extract/                         # 提取引擎
│   ├── router.py                    # Extractor 主类 → 统一分发 extract_generic
│   ├── schemas/
│   │   ├── __init__.py              # 导出 NodeExtractionResult 等
│   │   └── nodes.py                 # Pydantic schema（ExtractedNode/Edge）
│   ├── pipelines/
│   │   ├── _base.py                 # call_llm, parse_json 等工具函数
│   │   └── generic.py               # 统一 2-call 管线
│   └── prompts/
│       ├── __init__.py              # DOMAIN_PROMPTS 注册表
│       └── domains/
│           ├── __init__.py          # 领域注册表
│           ├── _base.py             # 共享模板（system prompt + user message 构建器）
│           ├── policy.py            # 政策域 — 9 种节点类型定义
│           ├── industry.py          # 产业域 — 6 种节点类型定义
│           ├── technology.py        # 技术域 — 5 种节点类型定义
│           ├── futures.py           # 期货域 — 6 种节点类型定义
│           ├── company.py           # 公司域 — 5 种节点类型定义
│           └── expert.py            # 专家域 — 4 种节点类型定义
│
├── verify/                          # 验证工具
...
│
├── web/
│   └── app.py                       # FastAPI Web UI（Node/Edge 渲染）
│
├── commands/
│   └── run_url.py                   # 单 URL CLI 命令
│
├── cli.py                           # CLI 入口
│
└── database/
